...
Sensitive inputs are a useful feature for passing secrets to your building blocks, but you should be aware of their limitations. Platform engineers should strongly consider if their use case can be replaced with Workload Identity Federation or an external key management system.
...
We currently do not support sensitive outputs. We suggest using an external secret management system like Azure Key Vault instead. Because we do not support sensitive outputs, we also do not support secret inputs.
