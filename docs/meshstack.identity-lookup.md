...
> The recommended way to set up [user provisioning via SCIM](meshstack.user-group-provisioning). The instructions below are not needed if users are provisioned via SCIM.
...
In order to use Azure lookup functionality, you must create a new service principal as described in [Azure Service Principal Setup](meshstack.azure.index#service-principal-setup) and assign the following required permissions as an **application permission**:
...
To provision new meshUsers as [guest users](administration.users#guest-users), configure the guest detection.
