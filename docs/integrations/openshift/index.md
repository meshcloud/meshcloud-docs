...
meshStack supports management of RedHat OpenShift platforms. OpenShift has a [Kubernetes](../kubernetes/index.md) core and provides additional services. It is available in both Open Source flavors (OKD) as well as enterprise offerings by RedHat.

meshStack supports project creation, configuration, access control, quota management and billing for OpenShift.

## Integration Overview

To enable integration with OpenShift, platform engineers configure one or multiple `meshPlatform`s of `PlatformType` OpenShift in meshPanel.
...
meshStack will identify and assign users in OpenShift via their euid (external user id) as described in [Identity Federation](../../concepts/identity-and-access-management.md#externally-provisioned-identities).
...
Openshift supports meshStack's powerful multi-cloud [tagging system](../../concepts/tag.md).
