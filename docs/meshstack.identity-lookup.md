...
let AzureCreds =
    {-
      Setting this configuration enables the use of Microsoft Entra ID as a user lookup source to allow
...
let AzureGuestDetection =
    {-
      ...
      workspace they are not detected as guest users.
        ...
        guestProperty:
            The Microsoft Entra ID's custom attribute which is checked against the guestValue.
...
let AzureEuidUserAttribute =
    {-
      ...
      workspace their euid's are not udpated.
        ...
        userAttributeName:
            The Microsoft Entra ID's user (custom) attribute.
    -}
