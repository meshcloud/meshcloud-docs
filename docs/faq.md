...
meshStack is hosted in leading cloud providers GCP - alternative AWS or Azure. The geolocation is primarily within the EU but can be changed upon the customer’s requirement. This includes also the storage for backups. meshStack leverages different services of the cloud to provide security in terms of data encryption, protection against DoS attacks, access control, etc.
...
The External User Identifier (like the Email Address) is replicated to the respective integrated cloud platforms, e.g. AWS, GCP, Azure, etc.
...
meshStack uses different cloud providers such as AWS, GCP, Azure, etc. in order to store and backup data. These data can be located in the data centers of the respective cloud providers mainly in the EU region as well as across the globe.
