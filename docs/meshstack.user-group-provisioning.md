...
We provide an SCIM API that enables external systems such as an Microsoft Entra ID to directly
...
the members of a certain Microsoft Entra ID group are changing and provisioning is set up, those changes will be reflected in the group within meshStack as well
...
From that response the Microsoft Entra ID retrieves the user's id and can from now on refer to this user with the identifier `54fbbbaa-c80c-49df-b13d-27133cb81740`.
Let's further assume that the Microsoft Entra ID wants to update the members of a group with the identifier `d90c1657-63b1-44be-8cdc-e3a8ffc5a7d1` and the aforementioned user
is the only member, then the Microsoft Entra ID will send the following PatchOperation:
...
The following guide shows how an Microsoft Entra ID can be configured to enable SCIM user and group provisioning to meshStack. In order
...
To set up the provisioning on Microsoft Entra ID side, have a look at [Microsoft's guideline](https://docs.microsoft.com/en-us/azure/active-directory/app-provisioning/configure-automatic-user-provisioning-portal) and please follow these steps:

1. [Enable SCIM](meshstack.user-group-provisioning.md#enable-scim) in meshStack.
2. Create a new non-gallery Enterprise Application (EA) in your Microsoft Entra ID that is dedicated to the provisioning. A step-by-step guide is available [here](https://docs.microsoft.com/en-us/azure/active-directory/app-provisioning/use-scim-to-provision-users-and-groups#getting-started).
...
4. Go to "Settings" menu within "Provisioning" and configure which users and groups should be provisioned. You can either:
   ...
   2. Sync all users and groups from your Microsoft Entra ID. This option should always be combined with Scopes as described in the step below.
...
6. In the Mapping for Users make sure that you have the mappings configured as described in the [user mappings table](#user-mappings-table) and remove all other mappings. Note that the externalId attribute should be mapped to the Microsoft Entra ID Attribute that is used as the euid in meshStack.
7. Start the provisioning process and regularly monitor the provisioning logs.

### User Mappings Table

| Microsoft Entra ID Attribute                                                 | SCIM app attribute (meshStack)             |
|---------------------------------------------------------------|--------------------------------------------|
...
(Microsoft Entra ID) configured as described above.

### Creation

Microsoft Entra ID sends a request to create a user via SCIM if that user is in the provisioning scope of the Enterprise Application.
...
- A user was explicitly disabled in Microsoft Entra ID.
- A user was removed in Microsoft Entra ID, but not permanently deleted yet: If the user is still visible in the "Deleted users"
  section in Microsoft Entra ID, then the user is deactivated, but not deleted.
...
- The user will be unable to log in to meshPanel. This is also the case when federated authentication is used with an
  ...
  authentication, users are unable to login if Microsoft Entra ID deactivated them via SCIM, regardless of their activation status in
  Google.
...
- The user was *permanently* deleted in Microsoft Entra ID.
- The user was previously deleted in Microsoft Entra ID, and has now automatically been permanently deleted in Microsoft Entra ID after a given
  period (usually 30 days) has passed.

**Attention**: A previous version of this guide has explicitly demanded to configure Microsoft Entra ID such that user deletion via
