...
1. Creating a [building block definition](administration.building-blocks) that deploys an AWS S3 Bucket using Terraform.
...
- Access to a meshStack workspace that has the [Platform Builder enabled](marketplace.platform-builder).  
...
6. **Entering Terraform implementation details**
   ...
   - Enable the option “Use meshStack’s Http Backend” if using our template.
     Learn more about how this meshStack backend works [here](meshstack.building-blocks.meshStack-http-backend).
...
7. **Entering inputs**
   ...
   > meshStack can work with [dependencies between building blocks](administration.building-blocks#dependencies).
