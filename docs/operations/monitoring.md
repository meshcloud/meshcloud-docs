...
Our [managed service](./managed-service.md) team
...
meshStack collects various logs as described in [Logging & Auditing](./logging.md). Logs can contain customer data and also PII like user identifiers.
...
When enabled, collected [product feedback and usage data](./product-feedback-collection.md) is relayed from meshPanel's web server to a central plausible server instance operated by meshcloud.
