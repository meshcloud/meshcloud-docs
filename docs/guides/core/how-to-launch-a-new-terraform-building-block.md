...
The Building Block *Definition* is the template/blueprint of Building Blocks. It describes what piece of e.g. OpenTofu or other infrastructure has to be rolled out.
...
- The OpenTofu runner executes the code defined in the Building Block, thereby provisioning platform resources.
- A GIT repository contains the OpenTofu code, which is maintained by the provider and utilized in the Building Block definition. This code specifies the resources that need to be provisioned on the platform.
- A Platform represents a target private or public cloud environment to provision resources described in the OpenTofu code.
...
A Building Block input is used to supply configuration parameters to OpenTofu code. Each input has following settings:

- The input N**ame** must correspond to the OpenTofu variables.
- While the **Display Name** field is optional, it enables the provision of a user-friendly name for the Building Block requester, distinguishing it from OpenTofu's naming conventions.
- **Source** defines how the input is provided to the OpenTofu: by the requester, platform engineer, static value or generated by meshStack
...
- **Provide as** feature enables users to specify the method of input delivery to the OpenTofu code, either as an environment variable or as a regular variable (typically defined in the `variables.tf` file)
- Finally, users can choose to encrypt the input's value by toggling the corresponding switch on, which is particularly useful when supplying credentials to OpenTofu.
...
However this code cannot run on the meshStack OpenTofu runner for clear reasons. The runner starts from a fresh environment and does not have any preconfigured AWS profiles.

More optimal solution would be to rely on environment variables `AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY`. In that case the OpenTofu configuration for AWS provider could look like:

```json
 provider "aws" {}
```

These variables must be passed by meshStack to the OpenTofu runner as input variables created in the Building Block definition as following:
...
This concept is not only limited to AWS, but applies to many OpenTofu providers that can use environment variables as credentials. Another example is the Datadog provider, which is configured similarly. The credentials are passed to the OpenTofu runner as Building Block Input environment variables, `DD_API_KEY` and `DD_APP_KEY`. Please refer to [https://registry.terraform.io/providers/DataDog/datadog/latest/docs](https://registry.terraform.io/providers/DataDog/datadog/latest/docs) for more details.

For implementing OpenTofu, meshStack will provide preconfigured inputs for setting up credentials, depending on the platform you've chosen for the Building block. For instance, if your building block runs on the AWS platform, select "Generate AWS auth inputs" from the "Generate Input" menu, as shown.
...
User permissions is a special type of input. It provides to the OpenTofu runner the list of all users assigned to the project including their mail address, first/last name and respective roles. The permissions are passed as JSON list as shown on the example below:
...
To handle such list in your OpenTofu code, define the following variable, usually in `variables.tf` file:
...
Finally, to process this variable use the OpenTofu code below in `main.tf` file. It defines three lists corresponding to each default project role:
...
The following example demonstrates how to create a list of user identities with only admin roles. This list can subsequently be utilized to grant permissions on resources provisioned through OpenTofu:
...
Generally, any output generated by OpenTofu code can be accessed by meshStack if required. A common example might be when OpenTofu creates a resource on a custom platform, which is considered a tenant for meshStack. In such a case, meshStack would need to know the tenant's ID. This can be done by creating a new output with the name that match OpenTofu output variable and having the following characteristics:

- **Type**: String
- **Assignment Type**: Platform Tenant ID

The example below demonstrates an output variable `tenant_id`. It will be initialized to the OpenTofu output of the same name following successful execution.
