...
meshKube is designed to give platform teams new to meshStack a ready-to-use developer portal implementation meant for Kubernetes.
This developer portal enables users to provision a ready-to-use, best-practice environment on Azure Kubernetes Service (AKS) with minimal setup.

## What is meshKube?

At its core, meshKube is an automated starterkit powered by a Terraform module.
...
The only requirement for users is to integrate a **GitHub organization** of their choice.
...
To create a meshKube instance, the module requires a few key inputs:

*   A `name` for the application, which is used for the projects, namespaces, and repository.
*   The `creator`'s identity to grant initial admin access.
